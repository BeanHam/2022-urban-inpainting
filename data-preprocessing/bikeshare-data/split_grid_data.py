...
def main():
    ...
    ## load data
    np.random.seed(1)
    root = "D:/nyc_bikeshare/grid_data/"
    
    ## get train data
    train_years = list(range(2019,2022))
    all_data = []
    ...
    ## split data
    train_data = all_data[:-30*24, :,:] 
    val_data = all_data[-30*24*3:-30*24, :,:] ## 3 months
    test_data = all_data[-30*24:, :,:] ## 1 month
    ...
    ## save data
    np.save(f"D:/nyc_bikeshare/data-3d/train.npy", train_data)
    np.save(f"D:/nyc_bikeshare/data-3d/test.npy", test_data)
    np.save(f"D:/nyc_bikeshare/data-3d/val.npy", val_data)

    ## save masks
    for key in masks.keys():
        np.save(f"D:/nyc_bikeshare/data-3d/{key}_mask.npy", masks[key])
