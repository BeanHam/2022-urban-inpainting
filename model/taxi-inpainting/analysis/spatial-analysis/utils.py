...
def train(dataset_train, 
    ...
    for mape_loss in mape_losses:
        ...
        for ite in tqdm(range(max_iter)):
            ...
            ## training
            for i in tqdm(range(chunk_size-1, len(dataset_train), batch_size)):
                inner_indices = range(i, min(len(dataset_train)-1, i+batch_size))
                mask, gt= zip(*[dataset_train[j] for j in inner_indices])
