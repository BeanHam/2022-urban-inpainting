...
class taxi_data(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        chunk_masks = torch.ones_like(chunk_imgs).float()
        chunk_masks[-1] = torch.from_numpy(self.masks[index]).float()
...
def train_l1(dataset_train, dataset_val, dataset_test_biased, dataset_test_random, parameters, all_results):
    ...
    for hole_loss_weight in hole_loss_weights:
        ...
        for i in tqdm(range(max_iter)):
            ...
            loss = hole_loss_weight*(loss_dict['l1_loss_current_hole'])+\
                   valid_loss_weight*(loss_dict['l1_loss_current_valid'])+\
                   prev_hours_loss_weight*(loss_dict['l1_loss_prev_hours'])
...
############################################################
################## train previous hours
############################################################   
def train_3d(dataset_train, dataset_val, dataset_test_biased, dataset_test_random, parameters, all_results):
    
    ## weight values values
    epsilon = parameters['epsilon']
    valid_loss_weight = parameters['valid_loss_weight']
    hole_loss_weight = all_results[parameters['mask_type']+'1d']['hole_loss_weight']
    prev_hour_loss_weights = parameters['prev_hour_loss_weights']
    parameters['hole_loss_weight'] = hole_loss_weight
    parameters['relative_loss_weight'] = 0
    ...
    counter = 1
    for prev_hour_loss_weight in prev_hour_loss_weights:
    
        parameters['prev_hour_loss_weight'] = prev_hour_loss_weight
        print(f'[{counter}/{len(prev_hour_loss_weights)}]...')
        ...
        ## check with best performing model so far
        if final_val_loss < best_model['best_val_perf']:
            best_model['prev_hour_loss_weight'] = prev_hour_loss_weight
            best_model['best_val_perf'] = final_val_loss
            best_model['val_model_loss'] = val_model_loss
            best_model['model_state'] = model.state_dict()
        counter += 1
...
def train_relative(dataset_train, dataset_val, dataset_test_biased, dataset_test_random, parameters, all_results):
    ...
    prev_hours_loss_weight = parameters['prev_hours_loss_weight']
    hole_loss_weights = parameters['hole_loss_weights']
    relative_loss_weights = np.array(parameters['relative_loss_weights'])*epsilon
...
def loss_eval(model, criterion_m, dataset, parameters):
...
def loss_eval_all(model, criterion_m, dataset, parameters):
    ...
    for i in range(0, len(permutation), batch_size):
        ...
        with torch.no_grad():
            output, _ = model(gt, mask)
        loss_dict = criterion_m(mask, output, gt, epsilon)
        model_loss_total += hole_loss_weight*(loss_dict['l1_loss_current_hole'])+\
                            valid_loss_weight*(loss_dict['l1_loss_current_valid'])+\
                            prev_hour_loss_weight*(loss_dict['l1_loss_prev_hours'])+\
                            relative_loss_weight*(loss_dict['relative_loss'])
...
def quantitative_eval(model, dataset, parameters):
    ...
    for i in range(0, len(permutation), batch_size):
        ...
        for j in range(len(output)):
            ...
            ## calculate metrics
            ## overall image
            ssim_output_5.append(structural_similarity(output_comp_single, gt_single, win_size=5))
            #ssim_output_7.append(structural_similarity(output_comp_single, gt_single, win_size=7))
            #ssim_output_9.append(structural_similarity(output_comp_single, gt_single, win_size=9))
    ...
    return {'hole_l1_output':hole_l1_output,
            ...
            #'ssim_output_7':ssim_output_7,
            #'ssim_output_9':ssim_output_9,
...
def process_results(results, chunk_size):
    ...
    for result in results:
        ...
        biased_tabular_view.append([
            ...
            #np.mean(result['test_metrics_biased']['ssim_output_7']),
            #np.mean(result['test_metrics_biased']['ssim_output_9']),
        ...
        random_tabular_view.append([
            ...
            #np.mean(result['test_metrics_random']['ssim_output_7']),
            #np.mean(result['test_metrics_random']['ssim_output_9']),
