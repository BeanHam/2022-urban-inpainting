...
class PConvUNet(nn.Module):
    def __init__(self, layer_size=4, input_channels=1, upsampling_mode='nearest'):
        ...
        ## encoders
        self.enc_1 = PCBActiv(input_channels, 64, sample='down-7',conv_bias=True)
        self.enc_2 = PCBActiv(64, 128, sample='down-5',conv_bias=True)
        self.enc_3 = PCBActiv(128, 256, sample='down-5',conv_bias=True)
        self.enc_4 = PCBActiv(256, 512, sample='down-3',conv_bias=True)
        for i in range(4, self.layer_size):
            name = 'enc_{:d}'.format(i + 1)
            setattr(self, name, PCBActiv(512, 512, sample='down-3',conv_bias=True))
            
        ## decoders        
        for i in range(4, self.layer_size):
            name = 'dec_{:d}'.format(i + 1)
            setattr(self, name, PCBActiv(512 + 512, 512, activ='relu',conv_bias=True))
